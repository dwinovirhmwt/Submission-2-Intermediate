...
import com.bangkit23dwinovirhmwt.storyhubsubmission2.DataDummy
import com.bangkit23dwinovirhmwt.storyhubsubmission2.MainDispatcherRule
...
import com.bangkit23dwinovirhmwt.storyhubsubmission2.getOrAwaitValue
...
class MainViewModelTest{
    ...
    @Mock
    private lateinit var storyHubRepository: StoryHubRepository

    @Test
    fun `when Get Story Should Not Null and Return Data`() = runTest {
        val dummyStoryResponse = DataDummy.generateDummy()
        val data: PagingData<ListStoryItem> = StoryPagingSource.snapshot(dummyStoryResponse.listStory)
        val expectedStories = MutableLiveData<PagingData<ListStoryItem>>()
        expectedStories.value = data
        Mockito.`when`(storyHubRepository.getStories()).thenReturn(expectedStories)

        mainViewModel = MainViewModel(storyHubRepository)
        val actualStories = mainViewModel.listStory.getOrAwaitValue()

        val differ = AsyncPagingDataDiffer(
            diffCallback = StoryHubListAdapter.DIFF_CALLBACK,
